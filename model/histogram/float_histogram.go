...
func (h *FloatHistogram) trimBucketsInZeroBucket() {
	...
	// We are abusing Compact to trim the buckets set to zero above. We
	// avoid an actual compaction by providing a very large maxEmptyBuckets
	// (which doesn't affect empty buckets at the start and end). However,
	// this will also remove empty buckets at the end and is overall a bit
	// dirty. It could be solved more efficiently and elegantly. But this
